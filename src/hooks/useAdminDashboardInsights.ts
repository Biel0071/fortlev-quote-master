...
const EMPTY: DashboardInsights = {
  summary: { salesToday: 0, salesWeek: 0, ticketAverage: 0, onlineUsers: 0, hotUsers: 0, pendingQuotations: 0 },
  charts: { salesDaily: [], intentDaily: [] },
  lists: { onlineNow: [], hotLeads: [], recentActivity: [], abandonedCarts: [], topViewedProducts: [], topSearchTerms: [] },
...
export function useAdminDashboardInsights() {
  ...
  useEffect(() => {
    ...
    const load = async (silent = false) => {
      ...
      setData((response ?? EMPTY) as DashboardInsights);
      setLoading(false);
    };
