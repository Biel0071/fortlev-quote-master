...
serve(async (req) => {
  ...
  try {
    ...
    let total = 0;
    const prepared = [] as Array<{ path: string; content: string }>;
    for (const f of list) {
      if (!f?.path) continue;
      const c = safeSlice(String(f.content ?? ""), perFileCap);
      total += c.length;
      if (total > totalCap) break;
      prepared.push({ path: String(f.path), content: c });
    }

    const system =
      "Você é um revisor técnico sênior. Produza um relatório ESTRUTURADO em JSON estrito.";
